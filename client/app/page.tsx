...
const PPAHVerification = () => {
      ...
      if (type === "ANALYSIS_RESULT") {
        ...
        if (typeof similarity === "number" && anchorBiometricRef.current) {
          if (similarity < 0.45) {
            ...
            if (lowBioCounter.current > 3)
              handleSecurityEvent("Identity Mismatch", 15);
          } else {
  ...
  const handleSecurityEvent = (reason: string, penalty: number) => {
    ...
    console.log(
      `[PPAH Security] Event: ${reason}, Penalty: ${penalty}, Current Score: ${trustScoreRef.current}`,
    );
    ...
    setTrustScore((prev) => {
      ...
      const newEMA =
        EMA_ALPHA * targetScore + (1 - EMA_ALPHA) * trustScoreEMA.current;
      ...
      if (newScore < 40 && !challengeActiveRef.current) {
        console.warn(
          `[PPAH Security] Trust score critically low (${newScore}%), triggering liveness challenge`,
        );
  ...
  const startCall = async (activeSessionId: string) => {
    ...
    socketRef.current.onmessage = async (event) => {
      ...
      if (msg.type === "offer") {
        ...
        socketRef.current?.send(
          JSON.stringify({ type: "identify", sessionId: activeSessionId }),
        );
  ...
  const triggerScreenFlashCheck = async () => {
    ...
    if (delta < 2.0) {
      console.warn(
        `[PPAH] PHYSICS FAIL: Face did not reflect screen light. Delta: ${delta}`,
      );
  ...
  const startMonitoring = (activeSid: string) => {
    ...
    const loop = async () => {
        ...
        try {
          ...
          if (ctx) {
            ...
            if (
              ...
              previousColorEntropyRef.current !== null
            ) {
              const brightnessDelta = Math.abs(
                currentBrightness - previousBrightnessRef.current,
              );
              const entropyDelta = Math.abs(
                currentColorEntropy - previousColorEntropyRef.current,
              );
              ...
              const isEnvironmentStable =
                brightnessVariance < BRIGHTNESS_VARIANCE_THRESHOLD;
              ...
              const timeSinceLastChange =
                Date.now() - lastBrightnessChangeRef.current;
              const isLikelyNaturalChange =
                timeSinceLastChange > NATURAL_CHANGE_TIMEOUT &&
                environmentStableCountRef.current > STABLE_COUNT_THRESHOLD;

              if (
                brightnessDelta > brightnessThreshold &&
                ...
                console.warn(
                  `[PPAH] Scene Shift Detected - Brightness: ${brightnessDelta.toFixed(1)}, Entropy: ${entropyDelta.toFixed(2)}, Variance: ${brightnessVariance.toFixed(1)}`,
                );
                aggressiveMode = true;
                ...
                if (brightnessDelta > brightnessThreshold * 1.5) {
                  console.log(
                    `[PPAH] Natural brightness change detected (gradual): ${brightnessDelta.toFixed(1)}`,
                  );
                }
              }
            }
            ...
            try {
              ...
              try {
                ...
                console.warn(
                  `[PPAH Network] Hash verification failed for segment ${seg}:`,
                  netErr,
                );
              ...
              console.error(
                `[PPAH Hash Error] Failed to compute hash for segment ${seg}:`,
                hashErr,
              );
          ...
          if (results.faceLandmarks.length > 0) {
            ...
            if (
              ...
              setTrustScore((prev) => {
                ...
                // EMA formula: newEMA = alpha * newValue + (1 - alpha) * previousEMA
                const newEMA =
                  EMA_ALPHA * targetScore +
                  (1 - EMA_ALPHA) * trustScoreEMA.current;
                const smoothedScore = Math.round(newEMA);
                trustScoreEMA.current = newEMA; // Update EMA state
      ...
      const combinedScore =
        trustFactor * 0.4 + networkFactor * 0.3 + envStability * 0.3;
      ...
      if (aggressiveMode) {
        ...
        if (lastLoggedIntervalRef.current !== nextDelay) {
          console.log(
            `[PPAH Adaptive Hash] Aggressive mode active, interval: ${nextDelay}ms`,
          );
        ...
        if (lastLoggedIntervalRef.current !== nextDelay) {
          console.log(
            `[PPAH Adaptive Hash] Moderate monitoring, combined score: ${combinedScore.toFixed(2)}, interval: ${nextDelay}ms`,
          );
          lastLoggedIntervalRef.current = nextDelay;
        }
      } else {
        ...
        if (lastLoggedIntervalRef.current !== nextDelay) {
          console.log(
            `[PPAH Adaptive Hash] Heightened monitoring, combined score: ${combinedScore.toFixed(2)}, interval: ${nextDelay}ms`,
          );
          lastLoggedIntervalRef.current = nextDelay;
        }
      }
    ...
    try {
      ...
      for (const device of videoInputs) {
        ...
        if (BANNED_DRIVERS.some((banned) => label.includes(banned))) {
          alert(
            `SECURITY ALERT: Virtual Camera Detected (${device.label}). Access Denied.`,
          );
          return false;
        }
      }
  ...
  const registerSecurityKey = async () => {
    try {
      ...
      const resp = await fetch(
        `${getBackendUrl()}/api/webauthn/register/options`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const verifyResp = await fetch(
        `${getBackendUrl()}/api/webauthn/register/verify`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const performWebAuthnLogin = async () => {
    ...
    try {
      const resp = await fetch(
        `${getBackendUrl()}/api/webauthn/login/options`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: userEmail }),
      ...
      const verifyResp = await fetch(
        `${getBackendUrl()}/api/webauthn/login/verify`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const startVerification = async () => {
    ...
    if (cam) {
      const res = await fetch(`${getBackendUrl()}/api/session/init`, {
        ...
        body: JSON.stringify({
          email: userEmail,
          webauthn_credential_id: webAuthnCredRef.current,
        }),
      });
  ...
  const signPacket = async (
    ...
    const key = await crypto.subtle.importKey(
      "raw",
      enc.encode(sessionKeyRef.current),
    ...
    const sig = await crypto.subtle.sign(
      "HMAC",
      key,
      enc.encode(`${sid}${segId}${hash}${score}`),
    );
  ...
  const handlePointerUp = useCallback(() => {
    ...
    const closest = corners.reduce((prev, curr) => {
      const prevDist = Math.hypot(
        prev.x - pipPosition.x,
        prev.y - pipPosition.y,
      );
      const currDist = Math.hypot(
        curr.x - pipPosition.x,
        curr.y - pipPosition.y,
      );
      ...
      const interval = setInterval(async () => {
        try {
          const res = await fetch(
            `${getBackendUrl()}/api/session/${remoteSessionId}/security-report?t=${Date.now()}`,
          );
          if (res.ok) {
            const data = await res.json();
  ...
  return (
    <div
      className="h-screen w-screen bg-black overflow-hidden relative font-sans select-none"
    ...
    >
      ...
      <video
        ...
        style={{
          filter:
            remoteTrustScore !== null && remoteTrustScore < 60
              ? "blur(20px)"
              : "none",
        }}
      />
      ...
      {/* 2. TOP BAR (AUTO HIDE) */}
      <div
        className={`fixed top-0 left-0 right-0 z-20 transition-all duration-300 transform ${showControls || !inCall ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"}`}
      >
        <div className="bg-linear-to-b from-black/70 to-transparent pt-4 pb-12 px-4 flex justify-between items-start">
          <div className="flex flex-col text-white/90">
            <div className="flex items-center gap-2">
              ...
              {inCall && (
                <div className="bg-blue-600/20 px-2 py-0.5 rounded text-[10px] text-blue-400 font-bold border border-blue-500/30">
                  ENCRYPTED
                </div>
              )}
            </div>
          ...
          {inCall && (
            <div className="flex flex-col items-end gap-1">
              {(remoteTrustScore !== null || isRemoteConnected) && (
                <div
                  className={`flex items-center gap-1.5 px-2 py-1 rounded-full backdrop-blur-md border ${(remoteTrustScore || 100) > 80 ? "bg-green-900/40 border-green-500/30 text-green-400" : "bg-red-900/40 border-red-500/30 text-red-400"}`}
                >
      ...
      {(step === "active" || step === "initializing") && (
        ...
        >
          ...
          <div className="absolute bottom-2 left-2 right-2 flex justify-between items-end">
            <div
              className={`p-1 rounded-full bg-black/40 backdrop-blur border border-white/10 ${faceDetected ? "text-green-400" : "text-red-400"}`}
            >
              {faceDetected ? <Lock size={10} /> : <AlertTriangle size={10} />}
            </div>
            <span
              className={`text-[10px] font-bold px-1.5 py-0.5 rounded bg-black/40 backdrop-blur ${trustScore > 80 ? "text-green-400" : "text-red-400"}`}
            >
              {trustScore}%
            </span>
          </div>
          ...
          {challengeActive && (
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center text-center p-2">
              <Shield
                className="text-yellow-500 mb-1 animate-bounce"
                size={24}
              />
      ...
      <div
        className={`fixed bottom-0 left-0 right-0 z-40 transition-all duration-300 transform ${showControls ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"}`}
      >
        <div className="bg-linear-to-t from-black/90 via-black/50 to-transparent pb-8 pt-12 px-6 flex justify-center items-center gap-6">
          {step === "active" && (
            <>
              <button
                onClick={toggleMute}
                className={`w-14 h-14 rounded-full flex items-center justify-center text-white transition-all shadow-lg active:scale-95 ${isMuted ? "bg-red-500" : "bg-white/10 backdrop-blur-md hover:bg-white/20"}`}
              >
              ...
              <button
                ...
                className="w-16 h-16 rounded-full bg-red-600 flex items-center justify-center text-white shadow-red-900/50 shadow-lg hover:bg-red-500 transition-all transform active:scale-95"
              >
